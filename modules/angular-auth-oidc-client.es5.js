...
AuthConfiguration.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
OidcSecurityStorage.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
BrowserStorage.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
OidcSecurityCommon.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
OidcSecurityValidation.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
AuthWellKnownEndpoints.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
AuthWellKnownEndpoints.ctorParameters = function() {
  return [{ type: HttpClient }, { type: AuthConfiguration }, { type: OidcSecurityCommon }];
};
...
var OidcSecurityCheckSession = /** @class */ (function() {
  ...
  OidcSecurityCheckSession.prototype.pollServerSession = function(clientId) {
    var _this = this;
    var /** @type {?} */ source = Observable.timer(3000, 3000)
        .timeInterval()
        .pluck('interval')
        .take(10000);
    source.subscribe(
...
OidcSecurityCheckSession.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
OidcSecuritySilentRenew.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
...
OidcSecurityUserService.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
OidcSecurityUserService.ctorParameters = function() {
  return [{ type: HttpClient }, { type: OidcSecurityCommon }, { type: AuthWellKnownEndpoints }];
};
...
var OidcSecurityService = /** @class */ (function() {
      ...
      if (!result.error) {
        ...
        if (
          ...
          if (_this.oidcSecurityValidation.validate_signature_id_token(id_token, _this.jwtKeys)) {
            // validate nonce
            if (
              _this.oidcSecurityValidation.validate_id_token_nonce(decoded_id_token, _this.oidcSecurityCommon.authNonce)
            ) {
  ...
  OidcSecurityService.prototype.createAuthorizeUrl = function(nonce, state, authorization_endpoint, prompt) {
    ...
    var /** @type {?} */ params = new HttpParams({ fromString: urlParts[1], encoder: new UriEncoder() });
  ...
  OidcSecurityService.prototype.runTokenValidation = function() {
    ...
    var /** @type {?} */ source = Observable.timer(5000, 3000)
        .timeInterval()
        .pluck('interval')
        .take(10000);
    source.subscribe(
...
OidcSecurityService.decorators = [{ type: Injectable }];
/**
 * @nocollapse
 */
